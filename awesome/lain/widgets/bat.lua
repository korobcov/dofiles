...
local function worker(args)
    ...
    function update()
        ...
        if present == "1"
        then
            local rate  = first_line(bstr .. "/current_now")

            local ratev = first_line(bstr .. "/voltage_now")

            local rem   = first_line(bstr .. "/charge_now")

            local tot   = first_line(bstr .. "/charge_full")

            bat_now.status = first_line(bstr .. "/status") or "Unknown"
            ...
            local time_rat = 0
            if bat_now.status == "Unknown"
            then
                time_rat = (tot - rem) / rate
            elseif bat_now.status == "Discharging"
--	elseif bat_now.status == "Unknown"
            then
                time_rat = rem / rate
            end
